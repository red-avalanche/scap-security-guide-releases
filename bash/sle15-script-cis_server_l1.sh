...
find  /etc/issue.d/  -type f  ! -group 0 -regextype posix-extended -regex '^.*$' -exec chgrp -L $newgroup {} \;
...
find  /etc/issue.d/  -type f  ! -user 0 -regextype posix-extended -regex '^.*$' -exec chown -L $newown {} \;
...
# Remediation is applicable only in certain platforms
if ( rpm --quiet -q nftables && ! (systemctl is-active firewalld &>/dev/null) && rpm --quiet -q kernel-default ); then
