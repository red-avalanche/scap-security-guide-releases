...
# Remediation is applicable only in certain platforms
if rpm --quiet -q gdm && { rpm --quiet -q kernel-default; }; then
...
# Remediation is applicable only in certain platforms
if ( ! (systemctl is-active nftables &>/dev/null) && ! (systemctl is-active ufw &>/dev/null) && rpm --quiet -q kernel-default ); then
...
# Remediation is applicable only in certain platforms
if ( rpm --quiet -q iptables && ! (systemctl is-active firewalld &>/dev/null) && rpm --quiet -q kernel-default ); then
...
if [ $var_network_filtering_service == iptables ]; then
  "$SYSTEMCTL_EXEC" unmask 'iptables-ipv6.service'
  "$SYSTEMCTL_EXEC" start 'iptables-ipv6.service'
  "$SYSTEMCTL_EXEC" enable 'iptables-ipv6.service'
fi
...
find  /var/log/  -perm /u+xs,g+xws,o+xwrt  -type f -regextype posix-extended -regex '.*' -exec chmod u-xs,g-xws,o-xwrt {} \;
...
# set value in all files if they contain section or key
for f in $(echo -n "/etc/systemd/coredump.conf.d/oscap-autoremedy.conf"); do
    if [ ! -e "$f" ]; then
        continue
    fi
...
# if section not in any file, append section with key = value to FIRST file in files parameter
if ! $found ; then
    file=$(echo "/etc/systemd/coredump.conf.d/oscap-autoremedy.conf" | cut -f1 -d ' ')
    mkdir -p "$(dirname "$file")"
...
# Remediation is applicable only in certain platforms
if rpm --quiet -q kernel-default && { rpm --quiet -q systemd; }; then
