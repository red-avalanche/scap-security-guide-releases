...
# Remediation is applicable only in certain platforms
if ( rpm --quiet -q grub2-common && rpm --quiet -q kernel || rpm --quiet -q kernel-uek ) && [ ! -d /sys/firmware/efi ] && { ! ( [ -f /.dockerenv ] || [ -f /run/.containerenv ] ); }; then
...
for included_file in ${included_files} ; do
    ...
    LC_ALL=C sed -i "/^\s*KexAlgorithms/Id" "$included_file"
done
