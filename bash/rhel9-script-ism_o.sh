...
# $ oscap xccdf generate fix --profile xccdf_org.ssgproject.content_profile_ism_o --fix-type bash xccdf-file.xml
#
...
# Remediation is applicable only in certain platforms
if ! grep -q aarch64 /proc/sys/kernel/osrelease; then
...
for interface in "${eth_interface_list[@]}"; do
    if grep -qi "ZONE=$firewalld_sshd_zone" "/etc/NetworkManager/system-connections/${interface}.nmconnection"; then
        nic_bound=true
        break;
    fi
done
...
if [ $nic_bound = false ];then
    ...
    if ! firewall-cmd --state -q; then
        ...
        # Otherwise, regular sed command will do.
        sed_command=('sed' '-i')
        if test -L "/etc/NetworkManager/system-connections/${interface}.nmconnection"; then
            sed_command+=('--follow-symlinks')
        fi
        ...
        # We search for the key string followed by a word boundary (matched by \>),
        # so if we search for 'setting', 'setting2' won't match.
        if LC_ALL=C grep -q -m 1 -i -e "^zone=\\>" "/etc/NetworkManager/system-connections/${interface}.nmconnection"; then
            escaped_formatted_output=$(sed -e 's|/|\\/|g' <<< "$formatted_output")
            "${sed_command[@]}" "s/^zone=\\>.*/$escaped_formatted_output/gi" "/etc/NetworkManager/system-connections/${interface}.nmconnection"
        else
            # \n is precaution for case where file ends without trailing newline
            cce="CCE-89175-4"
            printf '\n# Per %s: Set %s in %s\n' "$cce" "$formatted_output" "/etc/NetworkManager/system-connections/${interface}.nmconnection" >> "/etc/NetworkManager/system-connections/${interface}.nmconnection"
            printf '%s\n' "$formatted_output" >> "/etc/NetworkManager/system-connections/${interface}.nmconnection"
        fi
